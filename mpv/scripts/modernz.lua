-- ModernZhttps://github.com/Samillion/ModernZ)
--
  ...
  for n = 1, #elements do
    ...
    elseif element.type == "button" then
      ...
      if hovered and (contains(user_opts.hover_effect, "size") or contains(user_opts.hover_effect, "color")) then
        ...
        if
          element.name == "title"
          or element.name == "tc_left"
          or element.name == "tc_right"
...
layouts["modern"] = function()
  ...
  if track_nextprev_buttons then
    ...
    lo.geometry =
      { x = refX - (60 + (chapter_skip_buttons and 60 or 0)) - offset, y = refY - 40, an = 5, w = 30, h = 24 }
  ...
  if jump_buttons then
    ...
    lo.style = (user_opts.jump_icon_number and icons.jumpicons[user_opts.jump_amount] ~= nil) and osc_styles.control_2
  ...
  lo.geometry = {
    ...
    y = refY - 84,
    an = 9,
    w = 50 + (state.tc_ms and 30 or 0) + (show_remhours and 25 or 0),
    h = 20,
  }
  lo.style = osc_styles.time
  ...
  if user_opts.show_chapter_title then
    ...
    lo.geometry =
      { x = 73 + (state.tc_ms and 32 or 0) + (show_hours and 20 or 0), y = refY - 84, an = 7, w = 30, h = 20 }
    lo.style = osc_styles.time

    lo = add_layout("chapter_title")
    lo.geometry =
      { x = 86 + (state.tc_ms and 32 or 0) + (show_hours and 20 or 0), y = refY - 84, an = 7, w = 200, h = 20 }
  ...
  if ontop_button then
    ...
    lo.geometry = {
      x = osc_geo.w - 172 + (loop_button and 0 or 45) + (info_button and 0 or 45) + (fullscreen_button and 0 or 45),
      y = refY - 40,
      an = 5,
      w = 24,
      h = 24,
...
local function osc_init()
  ...
  ne.visible = (osc_param.playresx >= 850 - outeroffset) and user_opts.volume_control
  ne.content = function()
  ...
  ne.visible = (
    osc_param.playresx
    >= 750 - outeroffset - (user_opts.info_button and 0 or 100) - (user_opts.fullscreen_button and 0 or 100)
  )
  ne.tooltip_style = osc_styles.tooltip
  if user_opts.tooltip_hints then
    ne.tooltipF = function()
      return state.looping and locale.loopdisable or locale.loopenable
    end
  end
  ne.eventresponder["mbtn_left_up"] = function()
    if state.looping then
  ...
  if user_opts.tooltip_hints then
    ne.tooltipF = function()
      return mp.get_property("ontop") == "no" and locale.ontop or locale.ontopdisable
    end
  end
  ne.visible = (
    osc_param.playresx
    >= 850
      - outeroffset
      - (user_opts.loop_button and 0 or 100)
      - (user_opts.info_button and 0 or 100)
      - (user_opts.fullscreen_button and 0 or 100)
  )
  ne.eventresponder["mbtn_left_up"] = function()
  ...
  ne.visible = (
    ...
    >= 950
      - outeroffset
      - (user_opts.ontop_button and 0 or 100)
      - (user_opts.loop_button and 0 or 100)
      - (user_opts.info_button and 0 or 100)
      - (user_opts.fullscreen_button and 0 or 100)
  )
  ...
  ne.visible = (
    ...
    >= 1050
      - outeroffset
      - (user_opts.screenshot_button and 0 or 100)
      - (user_opts.ontop_button and 0 or 100)
      - (user_opts.loop_button and 0 or 100)
      - (user_opts.info_button and 0 or 100)
      - (user_opts.fullscreen_button and 0 or 100)
  ...
  ne.content = function()
    if user_opts.chapter_fmt ~= "no" and chapter_index >= 0 then
      request_init()
      local chapters = mp.get_property_native("chapter-list", {})
      local chapter_title = (chapters[chapter_index + 1] and chapters[chapter_index + 1].title ~= "")
          and chapters[chapter_index + 1].title
        or locale.na
...
local function validate_user_opts()
  ...
  local colors = {
    ...
    user_opts.windowcontrols_minmax_hover,
  }
